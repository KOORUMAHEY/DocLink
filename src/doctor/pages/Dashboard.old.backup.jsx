          ...
          <div className="grid gap-4 sm:gap-6 lg:grid-cols-3">
                  ...
                  <div className="space-y-2.5 sm:space-y-3">
                    {upcomingAppointments.slice(0, 5).map((appointment, idx) => (
                      <div
                        ...
                        className="group flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg sm:rounded-xl bg-white border-l-4 border-l-blue-500 border border-gray-200 hover:border-blue-300 hover:shadow-md sm:hover:shadow-lg transition-all duration-300 hover:bg-blue-50/30"
                      >
                        {/* Avatar */}
                        <Avatar className="h-12 w-12 sm:h-14 sm:w-14 ring-2 sm:ring-3 ring-blue-100 shadow-md flex-shrink-0">
                          <AvatarImage src={appointment.patientAvatar} />
                          <AvatarFallback className="bg-gradient-to-br from-blue-500 to-cyan-600 text-white font-bold text-base sm:text-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <p className="font-semibold text-gray-900 text-sm sm:text-base group-hover:text-blue-600 transition-colors truncate">
                          ...
                          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-gray-600">
                            <div className="flex items-center gap-1.5">
                              <Clock3 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-500 flex-shrink-0" />
                              <span className="font-medium">{appointment.timeSlot || 'TBD'}</span>
                            </div>
                            {appointment.reason && (
                              <>
                                <span className="text-gray-300 hidden sm:inline">•</span>
                                <span className="truncate max-w-[180px] hidden sm:inline text-gray-500">{appointment.reason}</span>
                              </>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                          <Button 
                            size="sm" 
                            variant="ghost" 
                            className="h-8 w-8 p-0 hover:bg-green-50 hover:text-green-700 transition-colors"
                            title="Call patient"
                          >
                          ...
                          <Button 
                            size="sm" 
                            variant="ghost" 
                            className="h-8 w-8 p-0 hover:bg-purple-50 hover:text-purple-700 transition-colors hidden sm:flex"
                            title="View details"
                          >
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <Card className="border-0 shadow-lg sm:shadow-xl overflow-hidden">
                <CardHeader className="border-b bg-gradient-to-r from-white to-gray-50/50 p-4 sm:p-5 md:p-6">
                  <CardTitle className="flex items-center gap-2 sm:gap-3 text-base sm:text-lg md:text-xl">
                    <div className="p-2 sm:p-2.5 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex-shrink-0">
                ...
                <CardContent className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-6">
                  {/* Completion Rate */}
                  <div className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br from-green-50/50 to-emerald-50/30 border border-green-100 hover:border-green-200 transition-all">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      <span className="text-xs sm:text-sm font-semibold text-gray-700">Completion Rate</span>
                    ...
                    <Progress value={stats?.rates.completion || 0} className="h-2 sm:h-2.5" />
                    <p className="text-[10px] sm:text-xs text-gray-600 mt-2 sm:mt-2.5 font-medium">
                  ...
                  <div className="grid gap-3 sm:gap-3.5">
                    {[
                      { id: 'total-appts', label: 'Total Appointments', value: stats?.totals.appointments || 0, icon: CalendarCheck, color: 'bg-blue-100 text-blue-700' },
                      { id: 'active-patients', label: 'Active Patients', value: stats?.totals.patients || 0, icon: UserCheck, color: 'bg-purple-100 text-purple-700' },
                      { id: 'pending', label: 'Pending', value: stats?.totals.pending || 0, icon: Clock, color: 'bg-orange-100 text-orange-700' },
                    ].map((item) => {
                      const ItemIcon = item.icon;
                      return (
                        <div key={item.id} className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br from-gray-50/60 to-white/40 border border-gray-150 hover:border-gray-200 hover:shadow-sm transition-all duration-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2.5 sm:gap-3 flex-1 min-w-0">
                              <div className={`p-2 sm:p-2.5 rounded-lg ${item.color} flex-shrink-0`}>
                                <ItemIcon className="h-4 w-4 sm:h-5 sm:w-5" />
                              </div>
                              <div className="min-w-0">
                                <p className="text-[10px] sm:text-xs text-gray-600 font-medium">{item.label}</p>
                                <p className="text-lg sm:text-xl font-bold text-gray-900 mt-0.5">{item.value}</p>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <Card className="border-0 shadow-lg sm:shadow-xl overflow-hidden">
                <CardHeader className="border-b bg-gradient-to-r from-white to-gray-50/50 p-4 sm:p-5 md:p-6">
                  <CardTitle className="flex items-center gap-2 sm:gap-3 text-base sm:text-lg md:text-xl">
                    <div className="p-2 sm:p-2.5 rounded-lg bg-gradient-to-br from-orange-500 to-red-500 flex-shrink-0">
                    ...
                    {[
                      ...
                      return (
                        <Link key={action.label} href={action.href}>
                          <Button className={`w-full h-auto flex-col gap-2 py-3 sm:py-4 md:py-5 bg-gradient-to-br ${action.color} text-white hover:opacity-90 shadow-md hover:shadow-lg transition-all duration-200 hover:-translate-y-1 font-medium text-[11px] sm:text-xs md:text-sm`}>
                            <ActionIcon className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" />
